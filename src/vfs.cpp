...
static int users_mkdir(const char* path, mode_t mode) {
    ...
    char username[256];
    if (sscanf(path, "/%255[^/]", username) != 1) return -EINVAL;
    
    if (getpwnam(username) != NULL) return -EEXIST;
...
static int users_rmdir(const char* path) {
    ...
    if (strchr(path + 1, '/') != NULL) return -EPERM;
    
    if (getpwnam(username) == NULL) return -ENOENT;
